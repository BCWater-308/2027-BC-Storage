...
<ul>
  ...
  <li><strong>Sustainability target.</strong> "Sustainability" here means GWE at each Groundwater Level RMS well stays at or above its <strong>WY {trough_year}</strong> level going forward — the basin's historic low, at which no sustainability indicator registered an undesirable result. Minimum thresholds (MTs) for the GSP would be set <em>lower</em> than WY {trough_year} to create operational range and margin in the event of a 3–5 year drought. The volumetric sizing in this dashboard uses the basin's average annual loss rate as the need, which is a stronger target than "stay above WY {trough_year}" alone — matching average loss keeps the basin near WY 2025 conditions, providing built-in drought cushion.</li>
